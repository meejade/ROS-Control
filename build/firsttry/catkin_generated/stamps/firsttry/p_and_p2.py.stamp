...
class ArmGripperController:
    def move_to_initial_joint_angles(self):
        ...
        if self._angles_are_close(current_joint_angles, initial_joint_angles):
            loginfo_color("Already at the initial place.", Colors.GREEN)
        else:
            loginfo_color("Go to the initial place.", Colors.GREEN)
        ...
        if message_parts[0] == "place":
            ...
            try:
                ...
                if int(robot_id) == int(self.robot_id):
                    ...
                    target_joint_angles = [-1.499, 0.015, 0.037, 0.017]
                    ...
                    loginfo_color("Moved to specified joint angles successfully.", Colors.GREEN)
                    self.move_gripper(0.01)  # Open gripper to release
                    loginfo_color("Place sequence completed, waiting for next command.", Colors.CYAN)
                    # 发送表示动作执行完成的话题消息
                    self.place_status_publisher.publish(f"Placement completed: {self.robot_id}")

            except (ValueError, IndexError):
                rospy.logwarn("Invalid place_object message received.")
    ...
    def _positions_are_close(self, position1, position2, tolerance=0.01):
        return(
    ...
    def move_arm_to_position(self,target_position):
        ...
        if plan_position:
            loginfo_color("Arm movement successful(position).",Colors.GREEN)
        else:
            loginfo_color("Arm movement failed(position).",Colors.YELLOW)
            current_pose = self.get_current_pose()
            if current_pose:
                rospy.logwarn(f"Target Arm Position: {target_position}")
        ...
        while not target_position_reached:
            ...
            if current_pose:
                ...
                rospy.logwarn("Failed to get current pose during check.")
                break
            rospy.sleep(0.1)

    def _poses_are_close(self, pose1, pose2, tolerance=0.02):
    ...
    def move_arm_to_pose(self, target_pose):
        ...
        while True:
            ...
            if plan_pose:
                loginfo_color("Arm movement successful(pose).", Colors.GREEN)
                break
            else:
                loginfo_color("Arm movement failed(pose). Retrying...", Colors.YELLOW)
                current_pose = self.get_current_pose()
                if current_pose:
                    loginfo_color(f"Current Arm Pose: {current_pose}",Colors.MAGENTA)
                    loginfo_color(f"Target Arm Pose: {target_pose}",Colors.MAGENTA)
                else:
                    rospy.logwarn("Failed to get current pose during retry.")
    ...
    def _angles_are_close(self, current_angles, target_angles, tolerance=0.1):
    ...
    def move_arm_to_joints(self, joint_angles):
        ...
        self.arm_group.clear_pose_targets()
        if plan_joints:
            loginfo_color("Moved to target joint angles successfully.", Colors.GREEN)
        else:
            loginfo_color("Failed to move to target joint angles.", Colors.RED)
            current_joint_values = self.arm_group.get_current_joint_values()
            if current_joint_values:
                rospy.logwarn(f"Current Joint Values: {current_joint_values}")
                rospy.logwarn(f"Target Joint Values: {joint_angles}")
    ...
    def move_gripper(self,target_gripper_value):
        ...
        try:
            ...
            rospy.loginfo(f"Current Gripper Joint Values: {current_gripper_value}")
            rospy.loginfo(f"Setting Gripper Position to: {target_gripper_value}")
            self.gripper_group.set_joint_value_target([target_gripper_value, 0])
            plan_gripper = self.gripper_group.go(wait=True)
            self.gripper_group.stop()
            if plan_gripper:
                loginfo_color("Gripper movement successful.",Colors.GREEN)
            else:
                loginfo_color("Gripper movement failed.",Colors.BLUE)
    ...
    #     home_joint_angles = [0.0, 0.0, 0.0, 0.0]
        ...
        while not rospy.is_shutdown():
            if self.start_action:
                if not self._positions_are_close(self.get_current_pose().position, self.target_position):
                    ...
                    loginfo_color("Action sequence completed, waiting for next command.", Colors.CYAN)
