...
def coordinates_transfer(plan):
    coord_dict = {}
    file_name = "/home/turtle/Service_1/src/firsttry/scripts/physical layout.csv"
    coord_df = pd.read_csv(file_name)
    ...
    for agv in plan:
        for agv_plan in plan[agv]:
            ...
            print(agv_plan.visit_point,",s=",agv_plan.status)
                ...
                for attribute in attributes:
                    ...
                    if variable[0] == "\"agv_energy\"":
                        ...
                        if variable[0] == " \"charging_time\"":
                            ...
                            track = variable[1][7]
                            schedule.visit_point = track+"_"+variable[2].split("\"")[0]
...
async def client_behavior():
    ...
    while True:
        if is_terminated:
            ...
            exit()
        try:
            async with websockets.connect('ws://192.168.0.4:8002/', ping_interval=200) as ws:
                # if buffer:
                #     i = buffer.pop(0)
                #     await ws.send(i)
                # for i in buffer:
                ...
                while buffer:
                    # print("1 buffer 0 ",buffer[0])
                    await ws.send(buffer[0])
                    print(f"Sent message: {buffer[0]}")
                    buffer.pop(0)
                    # if buffer:
                    #     print("2 buffer 0 ",buffer[0])
                    # else:
                    #     print("buffer empty")
                    message = await ws.recv()
                    print(f"Received message: {message}")
