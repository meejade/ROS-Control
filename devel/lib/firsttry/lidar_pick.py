...
class Lidar_detector:
    ...
    def get_consecutive(self,target_index,n,direction,valid_ranges,num_valid_ranges,tolerance,target_index_in_valid,num_ranges):
        ...
        for i in range(n):
            ...
            if abs(valid_ranges[idx]-valid_ranges[target_index_in_valid]<tolerance):
                ...
                values.append((valid_ranges[idx],idx))
            else:
                break
            # print(f"direction:test,consecutive_values:{values}")    
    ...
    def find_consecutive_values(self,data,target_index,num_consecutive,tolerance,valid_ranges,num_valid_ranges,target_index_in_valid):
            ...
            consecutive_values_forward = self.get_consecutive(target_index,num_consecutive,0,valid_ranges,num_valid_ranges,tolerance,target_index_in_valid,num_ranges)
            if len(consecutive_values_forward) == num_consecutive:
                ...
                if num_consecutive % 2 == 1:
                    ...
                    consecutive_values_left_half = self.get_consecutive(target_index,num_left_half,-1,valid_ranges,num_valid_ranges,tolerance,target_index_in_valid,num_ranges)
                    print(f"reverse:{consecutive_values_left_half}")
                    consecutive_values_right_half = self.get_consecutive(target_index,num_right_half,1,valid_ranges,num_valid_ranges,tolerance,target_index_in_valid,num_ranges)
                ...
                if len(conticonsecutive_values_bothway) == num_consecutive:
                    ...
                    consecutive_values_backward = self.get_consecutive(target_index,num_consecutive,-1,valid_ranges,num_valid_ranges,tolerance,target_index_in_valid,num_ranges)
    ...
    def callback(self,data):
        ...
        num_consecutive = 8  # 连续值的数量
        # print(data.ranges[0],",",data.ranges[1],",",data.ranges[2],",",data.ranges[3],",",data.ranges[4],",",data.ranges[5],",",data.ranges[6],",",data.ranges[7],",")
        num_range = len(data.ranges)
        # print(f"Totol num of ranges:{num_range}")
        valid_ranges = []
        # 筛选出在指定范围内的距离
        ...
        if valid_ranges:
            ...
            if num_consecutive % 2 == 1:
                ...
                middle_index = consecutive_values[(num_consecutive//2)-1][1]
                if data.ranges[middle_index+1]==middle_value:
                    middle_index += 0.5
            # print(f"middlw_index:{middle_index}")
            # print("------------------------")
            # print(middle_value)
            # print(middle_index)
            # print("------------------------")
            ...
            # 计算物体的笛卡尔坐标
            self.x = middle_value * np.cos(target_angle)-0.023
            self.y = middle_value * np.sin(target_angle)
            self.z = 0.15
            self.middle_index = middle_index * 1.0
    ...
    def lidar_detect(self):
        ...
        world_coordinate_msg.coordinates = [self.x,self.y,self.z,self.middle_index]
        print("x,y,z,index:",self.x,",",self.y,",",self.z,",",self.middle_index)
