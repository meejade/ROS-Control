...
from lidar_pick import Lidar_detector
...
def place_status_callback(msg, name):
	...
	if temp == "Placement completed":
		place_order[name] = True
	else:
		place_order[name] = False
...
def calculate_target_orientation(position1, position2, c, orientation):
	if c == 0:
		return calculate_pose_orientaiton(orientation)
	delta_x = position2.x-position1.x
	cos_theta = delta_x/c
	delta_y = position2.y-position1.y
	sin_theta = delta_y/c
...
def move_to_start(namespace:str, start_point):
	...
	angular = calculate_target_orientation(current_position[namespace], start_point, distance, current_orientation[namespace])
...
def move_to(namespace:str, origin, target):
	...
	angular = calculate_target_orientation(origin, target, distance, current_orientation[namespace])
	print("d=",distance,",a=",angular)
	goal = PoseStamped()
	goal.header.frame_id = "map"
	goal.header.stamp = rospy.Time.now()
	...
	rospy.sleep(translate_time+3)
	if float(target.waiting_time) > 1:
		if target.status == 1:
			# print("status:",target.status,"x:",target.x,"y:",target.y)
			print("status1")
			print(pick_order.keys())
			while not pick_order[namespace]:
				lidar_dict[namespace].lidar_detect()
			pick_order[namespace]=False
			# order = String()
			# order.data = "start_detect:3"
			# pick_pub_dict[namespace].publish(order)
		if target.status == 0:
			print("status0")
		if target.status == 2:
			# print("status:",target.status,"x:",target.x,"y:",target.y)
			order = String()
			order.data = "place:1"
			# if current_orientation[namespace].z > 0:
			# 	order.data = "place:1"
			# elif current_orientation[namespace].z < 0:
			# 	order.data = "place:2"
			place_pub_dict[namespace].publish(order)
	rospy.sleep(float(target.waiting_time) + float(target.charging_time)-2)
	...
	if req.a == "connect":
		...
		if len(names) != 2:
			...
			if names[0]=="request plan":
				...
				global pub_dict
				global sub_list
				global pick_pub_dict
				global place_pub_dict
				global lidar_dict
				global pick_order
				...
				lidar_temp = Lidar_detector(names[1])
				lidar_dict[names[1]] = lidar_temp
				topic_name = names[1]+'/detect_status'
				pick_pub = rospy.Publisher(topic_name, String, queue_size=10)
				pick_pub_dict[names[1]] = pick_pub
				topic_name = names[1]+'/place_object'
				place_pub = rospy.Publisher(topic_name, String, queue_size=10)
				place_pub_dict[names[1]] = place_pub
		...
		#		sub = rospy.Subscriber(topic_name, Odometry, odom_callback, callback_args='tb3_'+str(i))
				...
				topic_name = names[1]+'/action_status'
				print("topic name:",topic_name)
				p2_sub_pick = rospy.Subscriber(topic_name,String, action_status_callback, callback_args=names[1])
				pick_order[names[1]]=False
				topic_name = names[1]+'/place_status'
				print("topic name:",topic_name)
				p2_sub_place = rospy.Subscriber(topic_name,String, place_status_callback,callback_args=names[1])
				sub_list.append(sub)
				sub_list.append(p2_sub_pick)
...
if __name__ == "__main__":
	...
	# p2_sub_pick = rospy.Subscriber('/tb3_3/detect_status',String, detect_status_callback, callback_args='tb3_3')
	# p1_sub_place = rospy.Subscriber('/tb3_0/place_status',String, place_status_callback, callback_args='tb3_0')
	# p2_sub_place = rospy.Subscriber('/tb3_3/place_status',String, place_status_callback, callback_args='tb3_3')
	# rospy.sleep(1)
	# print(p1_pick)
	# print(p2_pick)
	# picker = String()
