...
class ArmGripperController:
    ...
    def move_to_initial_joint_angles(self):
        ...
        if self._angles_are_close(current_joint_angles, initial_joint_angles):
            loginfo_color("Already at the initial place.(tb3_3)", Colors.GREEN)
        else:
            loginfo_color("Go to the initial place.(tb3_3)", Colors.GREEN)
            self.move_arm_to_joints(initial_joint_angles)
        self.move_gripper(0.01)  # Open gripper to default position

    def scan_callback(self,data):
        self.angle_incre = data.angle_increment
    ...
    def __init__(self):
        ...
        self.robot_id = None
        self.arm_group = moveit_commander.MoveGroupCommander(name="arm",robot_description="tb3_3/robot_description", ns="tb3_3")
        self.gripper_group = moveit_commander.MoveGroupCommander(name="gripper",robot_description="tb3_3/robot_description", ns="tb3_3")
        ...
        rospy.Subscriber('/tb3_3/object_world_coordinates', ObjectWorldCoordinates_RobotID, self.coordinate_callback)
        # Listening to the place object command
        rospy.Subscriber('/tb3_3/place_object', String, self.place_object_callback)
        rospy.Subscriber('/tb3_3/scan',LaserScan, self.scan_callback)

        # Publish the status of the action
        self.action_status_publisher = rospy.Publisher('/tb3_3/action_status', String, queue_size=10)
        # Detect again if the target is not within the workspace
        self.start_detect_publisher = rospy.Publisher('/tb3_3/detect_status', String, queue_size=10)
        # Publish the status of placing
        self.place_status_publisher = rospy.Publisher('/tb3_3/place_status', String, queue_size=10)
    ...
    def coordinate_callback(self, msg):
        ...
        loginfo_color(f"coordinate_callback!!!!!!!!!!!!!!{coordinates}",Colors.RED)
        if len(coordinates) == 4:
            self.target_position = Point(coordinates[0], coordinates[1], coordinates[2])
            self.target_index = coordinates[3]
    ...
    def place_object_callback(self, msg):
        ...
        if message_parts[0] == "place":
            print("111")
            try:
                self.robot_id = message_parts[1]
                print("robot_id ", self.robot_id)
                if int("1") == int(self.robot_id):
                    # Define the target joint angles from the image
                    target_joint_angles = [-1.57, 0.82, 0, -0.82] # right
                    loginfo_color(f"Received place command for robot ID {self.robot_id}. Moving to specified joint angles.", Colors.GREEN)
                    self.move_arm_to_joints(target_joint_angles)
                    loginfo_color(f"Moved to specified joint angles successfully. Robot ID = {self.robot_id}", Colors.GREEN)
                    self.move_gripper(0.01)  # Open gripper to release
                    self.move_arm_to_joints([0.0, -1.0, 0.3, 0.7]) # move back to the init pose
                    loginfo_color(f"Place sequence completed, waiting for next command. Robot ID = {self.robot_id}", Colors.CYAN)
                    # 发送表示动作执行完成的话题消息
                    self.place_status_publisher.publish(f"Placement completed: {self.robot_id}")
                elif int("2") == int(self.robot_id):
                    # Define the target joint angles from the image
                    target_joint_angles = [1.57, 0.82, 0, -0.82]  # left
                    loginfo_color(f"Received place command for robot ID {self.robot_id}. Moving to specified joint angles.", Colors.GREEN)
                    self.move_arm_to_joints(target_joint_angles)
                    loginfo_color(f"Moved to specified joint angles successfully. Robot ID = {self.robot_id}", Colors.GREEN)
                    self.move_gripper(0.01)  # Open gripper to release
                ...
                rospy.logwarn(f"Invalid place_object message received. Robot ID = {self.robot_id}")
    ...
    def move_arm_to_position(self,target_position):
        ...
        while not target_position_reached:
            ...
            current_pose = self.get_current_pose()
            loginfo_color(f"Get Current Pose. Robot ID = {self.robot_id}",Colors.YELLOW)

            changes = [abs(getattr(current_pose.position, axis) - getattr(previous_pose.position, axis)) < stable_tolerance for axis in ['x', 'y', 'z']]

            if all(change < stable_tolerance for change in changes):
                stable_count += 1
            else:
                stable_count = 0
            ...
            if stable_count >= stable_count_threshold:
                loginfo_color("Position angles have been stablized", Colors.GREEN)
                break
            rospy.sleep(0.1)
    ...
    def move_arm_to_joints(self, joint_angles):
        loginfo_color(f"Moving to First Position. Robot ID = {self.robot_id}", Colors.RED)
    ...
    def grasp_object(self, target_position):
        loginfo_color(f"Grasping Object. Robot ID = {self.robot_id}",Colors.GREEN)
        angle_rotate = self.angle_incre * self.target_index
        print("angle_rotate = ", angle_rotate)
        print("angle_incre = ", self.angle_incre)
        if(angle_rotate > 3.14):
            angle_rotate -= 6.2832
        print("angle_rotate = ", angle_rotate)
        if angle_rotate != 0:
            self.arm_group.set_joint_value_target([angle_rotate,-1.0,0.3,0.7])
            plan_joints = self.arm_group.go(wait=True)
        rospy.sleep(7)
        self.move_arm_to_position(target_position)
        print("FINISH First Movement!")
        #rospy.sleep(0.1)
        #new_target = Point(target_position.x,target_position.y,target_position.z-0.09)
        #self.move_arm_to_position(new_target)
