...
class ImageConverter:
    ...
    def subscribe_to_topics(self):
        if self.rgb_sub is None:
            self.rgb_sub = rospy.Subscriber("/camera_tb3_3/color/image_raw/compressed", CompressedImage, self.rgb_callback)
    ...
    def process(self, image):
        ...
        # Red
        mask1 = cv.inRange(hsv, (0, 120, 120), (20, 255, 255))
        mask2 = cv.inRange(hsv, (150, 150, 100), (200, 220, 200))
        ...
        if self.rgb_image is not None:
            ...
            if self.depth_image is not None:
                ...
                if stabilized_depth_image is not None:
                    ...
                    invalid_mask = np.isnan(stabilized_depth_image)
                    depth_display = cv.normalize(stabilized_depth_image,None,0,255, cv.NORM_MINMAX)
                    depth_display = cv.convertScaleAbs(depth_display)
                    depth_display = cv.applyColorMap(depth_display,cv.COLORMAP_JET)
                    depth_display[invalid_mask] = [0,0,255]
